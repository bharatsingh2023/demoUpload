...
//    }
   // @Scheduled(cron = "0 0 0 * * ?") // Fire at midnight every day
        ...
        for (int serviceNumber = 1; serviceNumber <= 8; serviceNumber++) {
            try {
                switch (serviceNumber) {
                    ...
                    case 4: {
                        PaymentDto dto = new PaymentDto();
                        dto.setI_FROM_DATE(environment.getProperty("I_FROM_DATE_YV_LU_PCK_RATE"));
                        dto.setI_TO_DATE(LocalDate.now().toString());
                        dto.setWERKS(environment.getProperty("WERKS_YV_LU_PCK_RATE"));
